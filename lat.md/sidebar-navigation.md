...
The primary path tunnels to the remote **dashboard** (see [[main-process#SSH dashboard transport]]): [[src/main/ssh-remote.ts#sshResolveDashboardPort]] gives the per-profile dashboard port (default 9119; named profiles get a distinct offset) and [[src/main/ipc/register.ts#getSshDashboardSessionConfig]] / [[src/main/dashboard.ts#sshDashboardConnectionFromConfig]] / the send-message preamble call [[src/main/ssh-tunnel.ts#ensureSshTunnel]] with it. Model-library IPC resolves the active profile too, so background model reads do not retarget the single tunnel to the default dashboard while named-profile chat is preparing. On the gateway-only fallback (no dashboard), the tunnel instead targets the profile's `platforms.api_server.extra.port` via [[src/main/ssh-remote.ts#sshResolveApiServerPort]], which auto-allocates and persists a remote profile port when one is missing. Tunnel starts are serialized by target, and stale SSH process exit/error callbacks cannot clear a newer retargeted tunnel. Either way the port is profile-scoped, so a non-default profile never reuses the default profile's tunnel and answers as default.
